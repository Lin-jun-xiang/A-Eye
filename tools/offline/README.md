...
**注意 `-ss` 的位置**：`-ss` 放在 `-i` **前面**是快速搜尋，會吸附到最近的關鍵幀 ——
實測在這支影片上偏移了 5~6 秒。那會讓你分析到完全不同的時段而毫無察覺
（我在這個專案裡被它騙了三次，包括把「起步在 163 秒」誤判成「157 秒」）。
分析特定時段時 **`-ss` 一律放在 `-i` 後面**（精確搜尋，代價是要從頭解碼）：

```bash
ffmpeg -v error -i in.mp4 -ss 158 -t 14 ...      # ✅ 精確
ffmpeg -v error -ss 158 -i in.mp4 -t 14 ...      # ❌ 會吸附到關鍵幀
```
